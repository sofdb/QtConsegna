...
QList<NailsProduct*> Catalogo::filterByNameContains(const QString& searchText) const {
    QList<NailsProduct*> risultati;
    for (const auto& p : products) {
        QString nome = QString::fromStdString(p.ptr->getName());
        if (nome.contains(searchText, Qt::CaseInsensitive)) {
            risultati.append(p.ptr);
        }
    }
    return risultati;
}
