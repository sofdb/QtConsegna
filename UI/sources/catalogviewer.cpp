...
void CatalogViewer::aggiornaSelezione() {
    ...
    if (!prodotto || catalogo.findProductByName(prodotto->getName()) == -1) return;
...
void CatalogViewer::gestisciModifica() {
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        if (newIndex >= 0 && newIndex < prodottiFiltrati.size()) {
            ...
            if (nuovoProdotto && catalogo.findProductByName(nuovoProdotto->getName()) != -1) {
                aggiornaSelezione();
            }
        }
    }
...
void CatalogViewer::gestisciRimozione() {
    ...
    if (conferma == QMessageBox::Yes) {
        NailsProduct* prodotto = prodottiFiltrati[index];
        catalogo.removeProduct(catalogo.findProductByName(prodotto->getName()));
        JsonHandler::saveCatalogToFile(catalogo, "catalogo.json");
        refreshCatalog();
...
void CatalogViewer::applyFilters() {
    ...
    prodottiFiltrati = ProductFilters::filtra(
        catalogo,
        searchBar->text(),
    ...
    for (auto* prodotto : prodottiFiltrati) {
        listaProdotti->addItem(QString::fromStdString(prodotto->getName()));
    }
}
